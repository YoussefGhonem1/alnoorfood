...
import '../widget/pick_image_page.dart';
  ...
  var img = showCupertinoModalPopup<XFile?>(
    ...
    builder: (BuildContext context) => Transform.scale(
      ...
      child: CupertinoAlertDialog(
        title: Text(LanguageProvider.translate('inputs', 'pick'),style:  TextStyleClass.smallStyle(),),
        actions: <CupertinoDialogAction>[
          CupertinoDialogAction(
            onPressed: ()async{
              navP(AddImagePage(multiple: false,),then: (val)async{
                await delay(100);
                navPop(val);
              });
            },
            // child: Text('cancel',s TextStyleClass),
            child: Text(LanguageProvider.translate('inputs', 'camera'),style:  TextStyleClass.smallStyle(),),
          ),
          CupertinoDialogAction(
            onPressed: ()async{
              if(video){
            ...
            child: Text(LanguageProvider.translate('inputs', 'photo'),style:  TextStyleClass.smallStyle(),),
          )
        ],
      ),
    ),
